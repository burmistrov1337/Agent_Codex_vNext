...
print("Uploading shared bot_analytics package...")
upload_tree(ROOT / "bot_analytics", "/opt/telegram_adk_bot/app/bot_analytics")
upload_tree(ROOT / "bot_analytics", "/opt/max_bot/app/bot_analytics")
...
print("Uploading MAX bot modules...")
for rel in [
    "__init__.py",
    "config.py",
    "database.py",
    ...
    upload_file(ROOT / "max_bot" / rel, f"/opt/max_bot/app/max_bot/{rel}")
...
unit_files = [
    ("deploy/vps/max-bot-webhook.service", "/etc/systemd/system/max-bot-webhook.service"),
...
run("systemctl enable --now max-bot-webhook.service")
