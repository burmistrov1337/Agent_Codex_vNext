...
print("SERVICE_ACTIVE:", run("systemctl is-active max-bot-webhook.service || systemctl is-active max-bot.service || true"))
print("SERVICE_STATE:")
print(run("systemctl show -p ActiveState -p SubState -p MainPID max-bot-webhook.service 2>/dev/null || systemctl show -p ActiveState -p SubState -p MainPID max-bot.service"))
...
print("RECENT_LOGS:")
print(run("journalctl -u max-bot-webhook.service -n 20 --no-pager -o cat 2>/dev/null || journalctl -u max-bot.service -n 20 --no-pager -o cat | tail -n 20"))
ssh.close()
