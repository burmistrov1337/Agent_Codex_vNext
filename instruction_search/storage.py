        ...
        for candidate in candidates:
            ...
            for query_form in query_forms:
                for candidate_form in candidate_forms:
                    ...
                    if ratio > best_ratio:
                        best_ratio = ratio
        # Fallback: compare query against normalized tokens from long search body.
        # This helps when active_name/synonyms/inci are empty or noisy in source posts.
        if best_ratio < 0.72 and search_text:
            token_candidates = {token for token in search_text.split() if len(token) >= 4}
            for token in token_candidates:
                token_forms = {compact_query(token), translit_query(token)}
                for query_form in query_forms:
                    for token_form in token_forms:
                        if not query_form or not token_form:
                            continue
                        ratio = difflib.SequenceMatcher(None, query_form, token_form).ratio()
                        if ratio > best_ratio:
                            best_ratio = ratio
        if best_ratio >= 0.72:
